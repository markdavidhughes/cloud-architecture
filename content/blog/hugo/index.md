...
```
8. Now remove the `config.toml` that was generated by Hugo as this is replaced by the `config.yaml` file from the example site.
```bash
rm config.toml
```
...
```
10. Preview the example site by running the following and then browsing to `http://localhost:1313/`.
```bash
hugo server
```
